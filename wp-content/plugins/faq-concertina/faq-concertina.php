...
function faqconc_show_faqs( $atts ) {
	...
	if ( $faqs->have_posts() ) {
		...
		wp_localize_script( 'faqconc-script', 'faqconcvars', array ( 'speed' => $speed, 'hideothers' => $hide_others ) );

		// display the FAQs
		$current_ver = '1_4_2';
		$faq_concertina = $category ? '<div id="faqconc_' . $current_ver . '" class="faqconc ' . $category . '" role="tablist" aria-multiselectable="true">' :'<div id="faqconc_' . $current_ver . '" class="faqconc" role="tablist" aria-multiselectable="true">'; // Add category as class if it exists
 		...
 		while ( $faqs->have_posts() ) {
			...
			$faq_concertina .= '<strong>' . get_the_title() . '</strong>';
